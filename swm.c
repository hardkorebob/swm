...
static const struct {
  ...
  size_t size;
    ...
    {"terminal", offsetof(Config, terminal_cmd),
...
static void tile_add(Node *t, Window w) {
  if (t->tile.nwindows >= t->tile.win_cap) {
    ...
    Window *tmp = realloc(t->tile.windows, (size_t)new_cap * sizeof(Window));
    if (!tmp) {
...
static int xerr_detect(Display *d, XErrorEvent *e) {
  (void)d;
  (void)e;
  wm_detected = 1;
  return 0;
...
static void get_wm_name(Window wid, char *buf, int len) {
  ...
  if (XGetWindowProperty(dpy, wid, a_net_wm_name, 0, 256, False, a_utf8, &type,
                         &fmt, &ni, &after, &data) == Success &&
      data && ni > 0) {
  ...
  if (XGetWindowProperty(dpy, wid, XA_WM_NAME, 0, 256, False, XA_STRING, &type,
                         &fmt, &ni, &after, &data) == Success &&
      data && ni > 0) {
    snprintf(buf, len, "%.*s", (int)ni, (char *)data);
    XFree(data);
...
static void ensure_frame(Node *tile) {
  ...
  if (tile->tile.frame_win == None) {
    ...
    swa.event_mask = 0;
    Window w =
        XCreateWindow(dpy, root_win, fx, fy, (unsigned)fw, (unsigned)fh, 0,
...
static void read_cpu_raw(long *idle_out, long *total_out) {
  ...
  *total_out = 0;
  ...
  if (sscanf(buf, "cpu %ld %ld %ld %ld %ld %ld %ld %ld", &vals[0], &vals[1],
             &vals[2], &vals[3], &vals[4], &vals[5], &vals[6], &vals[7]) < 8)
    return;
...
static void bar_init(void) {
  ...
  status_bar_win =
      XCreateWindow(dpy, root_win, 0, by, (unsigned)sw, cfg.statusbar_height, 0,
                    depth, InputOutput, CopyFromParent,
                    CWBackPixel | CWOverrideRedirect | CWEventMask, &swa);
  XRaiseWindow(dpy, status_bar_win);
  XMapWindow(dpy, status_bar_win);
...
static void bar_draw(void) {
  ...
  XCopyArea(dpy, pm, status_bar_win, draw_gc, 0, 0, (unsigned)w, (unsigned)h, 0,
            0);
  XFreePixmap(dpy, pm);
  XFlush(dpy);
}
...
static void timebar_init(void) {
  ...
  timebar_win =
      XCreateWindow(dpy, root_win, 0, by, (unsigned)sw, cfg.timebar_height, 0,
                    depth, InputOutput, CopyFromParent,
                    CWBackPixel | CWOverrideRedirect | CWEventMask, &swa);
  XRaiseWindow(dpy, timebar_win);
  XMapWindow(dpy, timebar_win);
...
static void ensure_tab_bar(Node *tile) {
  ...
  if (tile->tile.tab_bar_win == None) {
    ...
    Window w =
        XCreateWindow(dpy, root_win, bx, by, (unsigned)bw, (unsigned)bh, 0,
                      depth, InputOutput, CopyFromParent,
                      CWBackPixel | CWOverrideRedirect | CWEventMask, &swa);
...
static void manage_window(Window wid) {
...

  {
    ...
    if (XGetWindowProperty(dpy, wid, a_net_wm_state, 0, 32, False, XA_ATOM,
                           &type, &fmt, &ni, &after, &data) == Success &&
        data && ni > 0) {
      Atom *atoms = (Atom *)data;
      for (unsigned long i = 0; i < ni; i++) {
...
static void action_next_workspace(void) {
  action_switch_workspace((cur_ws + 1) % NUM_WORKSPACES);
}
static void action_prev_workspace(void) {
...
static void action_close_window(void) {
  ...
  unsigned long ni, after;
  unsigned char *data = NULL;
  if (XGetWindowProperty(dpy, wid, a_wm_protocols, 0, 32, False, XA_ATOM, &type,
                         &fmt, &ni, &after, &data) == Success &&
      data && ni > 0) {
    Atom *atoms = (Atom *)data;
    int found = 0;
...
static void action_prev_tab(void) {
  ...
  if (tile->tile.nwindows > 1) {
    tile->tile.active_tab =
        (tile->tile.active_tab + tile->tile.nwindows - 1) % tile->tile.nwindows;
    arrange_tile(tile);
    focus_tile(tile);
  }
}
...
static void on_destroy_notify(XEvent *ev) {
